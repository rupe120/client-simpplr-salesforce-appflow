"""
Glue ETL Job: qna-answer
Q&A answers migration
"""
...
def transform_qna_answer(source_df, context, spark):
    """
    Transform Simpplr__Answer__c records to Zeus DB format
    ...
    # Create lookup UDFs
    lookup_questions_id = create_lookup_udf(context, "questions")
...
if __name__ == "__main__":
    run_etl_job(
        entity_name="qna-answer",
        s3_source_path="salesforce/Simpplr__Answer__c/",
        target_schema="content_mgmt",
        target_table="answers",
        transform_fn=transform_qna_answer
    )
